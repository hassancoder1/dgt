...
$company_names = [];
$khaata_ids_str = implode(",", array_unique([$fImporter['im_acc_kd_id'], $fExporter['xp_acc_kd_id'], $fNotify['np_acc_kd_id']]));
$query = mysqli_query($connect, "SELECT id, json_data FROM Khaata_details WHERE id IN ($khaata_ids_str)");
while ($row = mysqli_fetch_assoc($result)) {
    $json_data = json_decode($row['json_data'], true);
    $company_names[$row['id']] = $json_data['company_name'];
}
