<?php
$page_title = 'Sales Rem';
$pageURL = 'sale-remaining';
include("header.php");
...
$print_filters = [];
if ($_GET) {
    $remove = removeFilter('sale-remaining');
    $is_search = true;
...
$sql .= " AND locked = '1' AND transfer_level >= '4'";
    ...
    <div class="modal-dialog modal-fullscreen -modal-xl -modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">SALE REMAINING</h5>
...
<script>
    function viewPurchase(id = null, purchase_pays_id = null) {
        if (id) {
            ...
            $.ajax({
                url: 'ajax/viewSaleRemaining.php',
                type: 'post',
                data: {
                    id: id,
                    level: 1,
...
if (isset($_POST['transferRemToFull'])) {
    ...
    $data = array('transfer_level' => 6, '`from`' => 'sale-remaining');
...
if (isset($_POST['deleteRemPaymentAndRozSubmit'])) {
    ...
    $msg = 'DB Failed';
    $p_id_hidden = mysqli_real_escape_string($connect, $_POST['p_id_hidden']);
    $url_ = "sale-remaining?view=1&p_id=" . $p_id_hidden;
    ...
    if ($pays_del) {
        $msg = " Payment Deleted for Purchase #" . $_POST['s_sr'];
        $type = "success";
    }
