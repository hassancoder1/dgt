...
if ($loading_id > 0 && !empty($bill_of_entry_no)) {
    ...
    while ($SingleLoading = mysqli_fetch_assoc($Loadings)) {
        $id = $SingleLoading['id'];
        $_fields = transactionSingle($SingleLoading['p_id']);
        $agentDetails = json_decode($SingleLoading['agent_details'], true);

        // Verify if the bill_of_entry_no matches
...
";
                                ...
                                <tbody>
                                    ...
                                    ?>
                                            ...
                                            <td class="border border-dark text-center">
                                                <a href="carry-bill?edit=true&id=<?= $row['id']; ?>&loadingID=<?= $loading_id; ?>&bill_of_entry_no=<?= $bill_of_entry_no; ?>" class="text-primary me-2">
                                                    <i class="fa fa-edit"></i>
                                                </a>
                                                <?php if ($row['id'] !== $firstRowId): ?>
                                                    <a href="carry-bill?deleteAgPaymentEntry=true&id=<?= $row['id']; ?>" class="text-danger" onclick="return confirm('Are you sure you want to delete this record?');">
                        ...
                        <div class="card-body p-2">
                            <form method="post">
                                <div class="row gx-1 gy-3 table-form mb-3">
                                    <div class="col-2">
                                        <div class="input-group">
                                            ...
                                            <input value="<?= $Agent['ag_acc_no']; ?>" id="cr_acc"
                                ...
                                if (isset($transferDetails['transfer_info']) && $transferDetails['transfer_info'] != '') {
                                    $rozQ = fetch('roznamchaas', array('r_type' => 'Business', 'transfered_from_id' => $firstRow['id'], 'transfered_from' => 'agent_payments'));
                ...
                <div class="col-2 order-1 fixed-sidebar table-form" style="top: 60px !important;">
                    ...
                    <button class="btn btn-warning btn-sm mt-2" onclick="document.querySelector('.transfer-form').classList.toggle('d-none');">Transfer in Accounts</button>
