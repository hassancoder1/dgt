<?php $pageURL = 'transfer';
$page_title = 'Transfer Form ';
$table_head = array('TYPE', 'PURCHASER & SELLER', 'GOODS', 'REPORT', 'BAIL', 'IMPORTER', 'EXPORTER', 'NOTIFY PARTY');
include("header.php");
        ...
        <div class="card rounded-0">
            <div class="card-body p-0">
                ...
                <div class="table-responsive" style="height: 78dvh;">
                    <table class="table mb-0 table-sm table-bordered fix-head-table">
                        <thead>
                        <tr class="text-nowrap">
                            ...
                            <th>EXPORTER</th>
                            <th>NOTIFY PARTY</th>
                            <?php //foreach ($table_head as $item) {echo '<th>' . $item . '</th>';} ?>
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        while ($details = mysqli_fetch_assoc($query2)) {
                            ...
                            $transfer_as_msg = '';
                            if (empty($tware_json)) {
                                $rowColor = 'bg-danger bg-opacity-10 border border-danger';
                            } else {
                                //$transfer_as_msg = '<br><b>WH</b>' . $tware_json->comp_name;
                                if ($transfer_as <= 0) {
                                    $rowColor = 'bg-warning bg-opacity-10 border border-warning';
                                } else {
                                    $rowColor = '';
                                    $transfer_as_msg .= '<br><i class="fa fa-check-double text-success"></i> ';
                                    if ($transfer_as == 1) {
                            ...
                            <tr class="text-uppercase <?php echo $rowColor; ?>">
                                <td class="pointer text-nowrap"
                                    ...
                                    echo '<span class="font-size-11">' . $transfer_as_msg . '</span>'; ?>
                                </td>
...
<?php if (isset($_POST['savePartyPLoading'])) {
    ...
    if (is_numeric($d_id_hidden) && $d_id_hidden > 0 && in_array($type, ['Importer', 'Exporter', 'Notify', 'Warehouse', 'TWarehouse'])) {
        ...
        if ($source == 'purchase') {
            ...
            $done = update('sale_details', $data, array('id' => $d_id_hidden));
        }
        if ($done) {
            $msg = 'Party saved.';
            $msg_type = 'success';
...
<script>
    function viewDetails(id = null, d_id = null, source = null) {
        if (id) {
            $.ajax({
                url: 'ajax/viewSingleTransfer.php',
                type: 'post',
    ...
    <div class="modal-dialog modal-fullscreen -modal-xl -modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title bold" id="staticBackdropLabel"> TRANSFER DETAILS</h5>
