    ...
    <div class="modal-dialog modal-fullscreen -modal-xl -modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">BOOKIN PURCHASE ORDER DETAILS</h5>
...
if (isset($_POST['p_id_hidden_attach'])) {
    ...
    foreach ($_FILES["attachments"]["tmp_name"] as $key => $tmp_name) {
        if ($_FILES['attachments']['error'][$key] == 4 || ($_FILES['attachments']['size'][$key] == 0 && $_FILES['attachments']['error'][$key] == 0)) {
            ...
            $att = saveAttachment($ppp_id, 'purchase_order', basename($_FILES["attachments"]["name"][$key]));
...
if (isset($_POST['transferPurchase'])) {
    ...
    if ($locked) {
        $type = 'success';
        $msg = 'Booking Purchase transferred ';
    }
