...
include("header.php");
$goods_id = 0;
$remove = $size = $allot_name_get = $start_print = $end_print = $start = $end = $wh_khaata = $pur_sale = '';
$is_search = false;
if ($_GET) {
    ...
    if (isset($_GET['allot'])) {
        $allot_name_get = mysqli_real_escape_string($connect, $_GET['allot']);
        $pageURL .= '&allot=' . $allot_name_get;
    }
    ...
    <div class="col-lg-12">
        <form method="get" class="table-form d-flex align-items-center text-nowrap flex-fill">
            ...
            <div class="input-group w-50 d-none">
                <label for="wh_khaata">A/C</label>
                <select class="form-select" name="wh_khaata" id="wh_khaata">
            ...
            <select class="form-select" name="allot" id="allot" onchange="dataBranchDates();">
                ...
                foreach ($allotNames as $allotName) {
                    $allot_selected = $allotName == $allot_name_get ? 'selected' : '';
                    echo '<option '.$allot_selected.' value="' . $allotName . '">' . $allotName . '</option>';
                } ?>
            </select>
        ...
        <div class="d-flex align-items-center justify-content-between gap-3">
            ...
            <?php if ($is_search && $wh_khaata != '') {
                ...
                $popover = 'A/C NAME:' . $wh_khaata . ' COMPANY: ' . $dummy_wh_khaata['comp_name'];
                echo '<button type="button" class="btn btn-link p-0" data-bs-toggle="popover"
                    data-bs-trigger="focus" title="" data-bs-content="' . $popover . '"
                    data-bs-original-title="">' . $wh_khaata . ' Details
            </button>';
            } ?>
                    ...
                    <table class="table mb-0 fix-head-table">
                        ...
                        while ($details = mysqli_fetch_assoc($query_union)) {
                            ...
                            if ($is_search) {
                                ...
                                if ($allot_name_get != '') {
                                    if ($allot_name_get != $details['allot_name']) continue;
                                }
                                if ($start != '') {
                                    if ($source_table == 'purchase') {
                            ...
                            <tr class="text-uppercase text-nowrap font-size-11 <?php echo $rowColor; ?>">
                                <!--onclick="viewPurchase(<?php /*echo $purchase_id; */ ?>,<?php /*echo $pd_id; */ ?>)" data-bs-toggle="modal" data-bs-target="#KhaataDetails"-->
                                <td><?php echo $no; ?></td>
                                <td><?php echo $label . $parent_id . '-' . $d_sr; ?></td>
                                <td><?php echo $purchase_sale_date; ?></td>
                                <td><?php echo $p_khaata_no; ?></td>
                                <td><?php echo $s_khaata_no; ?></td>
                                <td><?php echo $details['allot_name']; ?></td>
