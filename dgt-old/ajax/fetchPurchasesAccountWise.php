...
if (isset($_POST['khaata_id'])) {
    ...
    $p_details_query = fetch('purchase_details');
    while ($details = mysqli_fetch_assoc($p_details_query)) {
        ...
        $purchases = fetch('purchases', array('id' => $parent_id));
        $sale = mysqli_fetch_assoc($purchases);
        ...
        if ($s_khaata_id == $khaata_id) {
            ...
            $output .= '<td>' . $parent_id . '-' . $details['d_sr'] . purchaseSpecificData($parent_id, 'purchase_type') . '</td>';
            $output .= '<td>' . $details['allot_name'] . '</td>';
            $output .= '<td>NILL</td>';
            $output .= '<td>' . my_date($sale['p_date']) . '</td>';
            ...
            $output .= '<td>' . round($details['total_qty_kgs']) . '</td>';
