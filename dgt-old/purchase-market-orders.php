<?php $page_title = 'Purchase Orders [MARKET]';
$pageURL = 'purchase-market-orders';
include("header.php"); ?>
<?php $remove = $size = $brand = $origin = $goods_name = $start = $end = $is_transferred = $s_khaata_id = '';
$is_search = false;
global $connect;
$sql = "SELECT * FROM `purchases` WHERE type = 'market' AND is_locked =0 ";
if ($_GET) {
...
<div class="row">
    <div class="col-lg-12">
        ...
        <div class="d-flex align-items-center justify-content-between gap-3">
            <?php echo addNew('purchase-market-add', '', 'btn-sm'); ?>
    ...
    <div class="modal-dialog modal-fullscreen -modal-xl -modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">MARKET PURCHASE ORDER DETAILS</h5>
...
<?php if (isset($_POST['deletePurchase'])) {
    ...
    if ($done) {
        $msg = " Deleted Market Purchase #" . $p_id_hidden;
        $type = "success";
    }
...
if (isset($_POST['p_id_hidden_attach'])) {
    ...
    foreach ($_FILES["attachments"]["tmp_name"] as $key => $tmp_name) {
        if ($_FILES['attachments']['error'][$key] == 4 || ($_FILES['attachments']['size'][$key] == 0 && $_FILES['attachments']['error'][$key] == 0)) {
            ...
            $att = saveAttachment($ppp_id, 'purchase_market_order', basename($_FILES["attachments"]["name"][$key]));
...
if (isset($_POST['transferPurchase'])) {
    ...
    if ($locked) {
        $type = 'success';
        $msg = 'Market Purchase transferred ';
    }
