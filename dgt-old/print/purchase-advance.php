<?php if (isset($_GET['start']) && isset($_GET['end']) && isset($_GET['goods_name']) && isset($_GET['ps_type'])
    && isset($_GET['is_transferred']) && isset($_GET['s_khaata_id'])
) {
    $backURL = '../purchases';
    require("../connection.php");
    global $connect;
    $sql = "SELECT * FROM `purchases` WHERE is_locked = 1 AND transfer =1 ";
    ...
    <head>
        <meta charset="UTF-8">
        <title>Purchase_Advance_<?php echo date('Y_m_d'); ?></title>
        ...
        <div class="row justify-content-center ">
            <div class="col-12">
                <div class="d-flex align-items-center justify-content-between mb-1">
                    ...
                    <div class="text-end">
                        <h2 class="fw-bold mb-0 text-uppercase">Purchase Advance</h2>
                ...
                <table class="table mb-0 table-bordered table-sm fix-head-table">
                    ...
                    while ($purchase = mysqli_fetch_assoc($purchases)) {
                        ...
                        <tr>
                            ...
                            <td class="small">
                                <?php echo '<b>' . $purchase['pct_amt'] . '[' . round($purchase['pct'], 2) . '%]</b><br>';
                                $adv_paid = purchaseSpecificData($purchase_id, 'adv_paid_total');
                                echo '<span class="text-success"><b>Paid </b>' . round($adv_paid) . '</span><br>';
                                echo '<span class="text-danger"><b>Bal. </b>' . round($purchase['pct_amt'] - $adv_paid) . '</span>';
                                ?>
                            </td>
                            <td class="font-size-11">
                                <?php if ($transfer2 == 1) {
                                    echo '<i class="fa fa-check-double text-success"></i> Transferred';
                                    echo '<br><span class="font-size-11"><b>Transfer D.</b>' . date('y-m-d', strtotime($purchase['t2_date'])) . '</span>';
                                } ?>
                            </td>
                        </tr>
                        <?php $row_count++;
