...
function generateSubMenu($parentId, $connect)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<li><a href="' . $row['url'] . '">' . $row['label'] . '</a>';
            generateSubMenu($row['id'], $connect); // Recursively generate sub-menus
            echo '</li>';
        }
        echo '</ul>';
    }
...
function generateSubMenuAdmin($parentId, $connect)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<a class="fw-bold-" href="navbar?id=' . $row['id'] . '">';
            $eva = $row['icon_class'] == '' ? '' : $row['icon_class'];
            echo '<i class="icon nav-icon" style="width:15px; height:15px; fill:#b4b2b2" data-eva="' . $eva . '"></i> ';
            echo '<span class="">' . $row['position'] . '</span>. ' . $row['label'] . '</a>';
...
function getPurchaseDetailsSerial($purchase_id)
{
    $serial = fetch('purchase_details', array('parent_id' => $purchase_id));
    $pd_serial = mysqli_num_rows($serial);
    return $pd_serial + 1;
}
...
{
    $removeFilter = '<a href="' . $returnUrl . '"><span class="ms-1 btn btn-outline-danger py-0 px-1 font-size-10 btn-sm"><i class="fa fa-sync-alt"></i>&nbsp;Clear</span></a>';
    return $removeFilter;
}
...
function backUrl($backUrl, $label = null, $btn_class = null, $i_class = null)
{
    ...
    $i_class = $i_class ? $i_class : ' bx bx-arrow-back ';
...
function addNew($url, $label = null, $btn_class = null)
{
    ...
    $a = '<a href="' . $url . '" class="btn btn-primary ' . $btn_class . '"><i class="bx bx-plus me-1"></i>' . $text . '</a>';
    return $a;
}
...
function isPurchaseBookingDetailsAdded($purchaseId)
{
    global $connect;
...
function khaataDetails($khaata_details_id, $static_type_no = null)
{
    $info = '';
    $kdd = fetch('khaata_details', array('id' => $khaata_details_id));
    if (mysqli_num_rows($kdd) > 0) {
        $kd_data = mysqli_fetch_assoc($kdd);
        $khaata = khaataSingle($kd_data['khaata_id']);
        $khaata_no = $khaata['khaata_no'];
        $static_type_no = isset($static_type_no) ? $static_type_no : 1;
        switch ($static_type_no) {
            case 1:
                if (SuperAdmin()) {
                    $array = array(array('t' => 'A/c.', 'v' => $khaata_no), array('t' => 'Owner', 'v' => $khaata['khaata_name']), array('t' => 'Company', 'v' => $kd_data['comp_name']), array('t' => 'Report', 'v' => $kd_data['report']));
                } else {
                    $array = array(array('t' => 'Company', 'v' => $kd_data['comp_name']));
                }
                break;
        }
        foreach ($array as $item) {
            $info .= '<span class="text-muted">' . $item['t'] . '</span><span class="text-dark font-size-11 bold">' . $item['v'] . '</span>';
        }
    }
    return $info;
}
...
{
    $query = fetch('purchases', array('id' => $purchaseId));
    $purchase = mysqli_fetch_assoc($query);
    $pur_d_q = fetch('purchase_details', array('parent_id' => $purchaseId));
    ...
    switch ($type) {
        ...
        case 'product_details':
            if (isPurchaseBookingDetailsAdded($purchaseId)) {
    ...
    switch ($type) {
        ...
        case 'product_details':
            if (isSaleDetailsAdded($saleId)) {
                ...
                $data = array('Qty' => $qty_no, 'KGs' => $total_kgs, 'Amount' => $amount, 'Final' => $total_amount, 'Goods' => $goods,
                    'Size' => $size,
                    'Brand' => $brand,
                    'Origin' => $origin,
                    'Allot' => $allot,
...
function bailDetails($details_id, $purchaseSale, $ps_type = null): array
{
    ...
    if (!in_array($purchaseSale, $allowedValues)) {
        ...
        if ($ps_type && $ps_type == 'local') {
            $data = array(
                'bail_report' => '', 'loading_date' => date('Y-m-d'), 'receiving_date' => date('Y-m-d'),
                'driver_name' => '', 'driver_phone' => '', 'truck_no' => ''
            );
            if (!empty($bail_json)) {
                $data = array(
                    'bail_report' => $bail_json->bail_report, 'loading_date' => $bail_json->loading_date,
                    'receiving_date' => $bail_json->receiving_date, 'driver_name' => $bail_json->driver_name,
                    'driver_phone' => $bail_json->driver_phone, 'truck_no' => $bail_json->truck_no
                );
            }
        } else {
            $data = array(
                'bail_report' => '', 'loading_country' => '', 'loading_port' => '', 'receiving_country' => '', 'receiving_port' => '',
                'loading_date' => date('Y-m-d'), 'receiving_date' => date('Y-m-d'),
                'freight' => '', 'bail_no' => '', 'container_name' => '', 'container_no' => '', 'container_size' => '', 'ship_phone' => '',
                'ship_email' => '', 'ship_wa' => '', 'ship_comp' => '', 'loading_shipper_address' => '', 'receiving_shipper_address' => ''
            );
            if (!empty($bail_json)) {
                $data = array(
                    'bail_report' => $bail_json->bail_report, 'loading_country' => $bail_json->loading_country, 'loading_port' => $bail_json->loading_port,
                    'receiving_country' => $bail_json->receiving_country, 'receiving_port' => $bail_json->receiving_port,
                    'loading_date' => $bail_json->loading_date, 'receiving_date' => $bail_json->receiving_date,
                    'freight' => $bail_json->freight, 'bail_no' => $bail_json->bail_no, 'container_name' => $bail_json->container_name,
                    'container_no' => $bail_json->container_no, 'container_size' => $bail_json->container_size, 'ship_phone' => $bail_json->ship_phone,
                    'ship_email' => $bail_json->ship_email, 'ship_wa' => $bail_json->ship_wa, 'ship_comp' => $bail_json->ship_comp,
                    'loading_shipper_address' => $bail_json->loading_shipper_address, 'receiving_shipper_address' => $bail_json->receiving_shipper_address
                );
            }
        }
    }
...
function loadingData($purchase_details_id)
{
    ...
    $data = array('goods_name' => $details['goods_name'], 'brand' => $details['brand'], 'origin' => $details['origin'],
        'size' => $details['size'], 'qty_name' => $details['qty_name'], 'qty_no' => $details['qty_no'], 'kgs' => $details['kgs'],
        'total_kgs' => $details['total_kgs'], 'saaf_wt' => $details['saaf_wt'], 'empty_wt' => $details['empty_wt'],
        'total_empty_wt' => $details['total_empty_wt'], 'loading_report' => $p_data['loading_report'],
        'loading_country' => $p_data['loading_country'], 'loading_port' => $p_data['loading_port'],
        'receiving_country' => $p_data['receiving_country'], 'receiving_port' => $p_data['receiving_port'], 'bail_no' => '', 'bail_report' => '',
        'loading_shipper_address' => '', 'receiving_shipper_address' => '', 'ship_phone' => '', 'ship_email' => '', 'ship_wa' => '',
        'ship_comp' => '', 'container_name' => '', 'container_no' => '', 'container_size' => '', 'loading_date' => $p_data['loading_date'],
        'receiving_date' => '', 'freight' => '', 'rowColor' => 'bg-danger bg-opacity-25',);
    if (!empty($json_loading)) {
