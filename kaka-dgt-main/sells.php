...
<div class="heading-div d-flex justify-content-between align-items-center flex-wrap grid-margin px-4 py-1 border-bottom">
    <div>
        <h4 class="mb-3 mb-md-0 d-none d-md-block">فروشی فارم تفصیل</h4>
        <h4 class="mb-3 mb-md-0 d-block d-md-none">فروشی</h4>
    </div>
    <div class="d-flex align-items-center flex-wrap text-nowrap">
        <form name="datesSubmit" method="POST" class="d-flex">
            ...
            <div class="flatpickr wd-md-100 wd-80 mb-2 mb-md-0" id="flatpickr-date">
                <input id="date_end" name="date_end" value="<?php echo $end_date; ?>"
                       type="text" class="form-control bg-transparent border-primary"
                       placeholder="تاریخ انتہاء" data-input>
            </div>
        </form>
        <form action="print/buys-and-sells" method="post" target="_blank" class="d-none">
            ...
            <input name="type" value="buys" type="hidden">
            ...
            <div class="table-responsive scroll screen-ht">
                <table class="table table-bordered " id="fix-head-table">
                    ...
                    <?php while ($loading = mysqli_fetch_assoc($records)) {
                        $cr_khaata_json = json_decode($loading['cr_khaata_json']);
                        $jmaa_khaata_noo = empty($cr_khaata_json) ? '' : $cr_khaata_json->jmaa_khaata_no;
                        $bnaam_khaata_noo = empty($cr_khaata_json) ? '' : $cr_khaata_json->bnaam_khaata_no;
                        ?>
                        <tr>
                            <td>
                                <div class="d-flex justify-content-between align-items-center">
                                    <a href="sells-add?id=<?php echo $loading["id"]; ?>"><?php echo $loading["id"]; ?></a>
                            ...
                            <td><?php echo $bnaam_khaata_noo;//$loading['bnaam_khaata_no']; ?></td>
