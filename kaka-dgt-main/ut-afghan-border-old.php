...
if ($_POST) {
    $removeFilter = '<a href="ut-afghan-border"><span class="ms-1 badge bg-danger urdu"><i class="mdi mdi-close-circle"></i> فلٹر ختم کریں</span></a>';
...
$sql = "SELECT * FROM ut_bail_entries WHERE is_surrender = 1 AND border_user_ids != '' {$username_append} {$date_append}";
...
<div class="heading-div d-flex justify-content-between align-items-center flex-wrap grid-margin px-4 py-1 border-bottom">
    <div><h4 class="mb-3 mb-md-0"> افغان بارڈر کسٹم کلئیرنگ </h4></div>
                ...
                <div class="table-responsive scroll screen-ht">
                    <table class="table table-bordered " id="fix-head-table">
                        ...
                        <tr>
                            ...
                            <th>امپورٹر</th>
                            <th>تاریخ بارڈر</th>
                            <th class="small-2">جی ڈی نمبر<br>ٹرک</th>
                            <th>ڈرائیور</th>
                            <th>باردانہ</th>
                            <th class="small-2" width="15%">رپورٹ</th>
                        </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr class="text-nowrap_">
                                <td>
                                    ...
                                    <?php if (empty($loading['border_json'])) {
                                        echo '<i class="fa fa-info-circle" data-tooltip="بیل نمبر ' . $loading["id"] . ' میں ابھی افغان کلئیرنگ کسٹم کی انٹری نہیں ہوئی۔ " data-tooltip-position="right"></i>'; ?>
                                    <?php } else { ?>
                                        <a href="#" class="btn btn-primary py-0 px-1 small"
                                           data-tooltip="قندھار کلئیرنگ کسٹم میں ٹرانسفر کریں"
                                           data-tooltip-position="left"
                                           onclick="transferUTAfghanBorder(this)"
                                           id="<?php echo $loading['id']; ?>"
                                           data-url="ut-afghan-border"
                                ...
                                ?>
                                <td><?php echo $borderJson['border_receiving_date']; ?>
                                    <br><?php echo $borderJson['border_unloading_date']; ?>
                                </td>
                                <td>
                                    <?php echo $borderJson['border_gd_no']; ?>
                                    <br><?php echo $borderJson['border_truck_no']; ?>
...
<script>
    function transferUTAfghanBorder(e) {
        var id = $(e).attr('id');
        var jins = $(e).attr('data-jins');
        var url = $(e).attr('data-url');
        var str = "کیا آپ قندھار کلئیرنگ کسٹم میں ٹرانسفر کرنا چاہتے ہیں؟";
        if (id) {
            if (confirm(str + '\n بیل نمبر:' + id + '\nجنس: ' + jins)) {
                window.location.href = 'ajax/transferUTAfghanBorder.php?id=' + id + '&url=' + url;
            } else {
