<?php include("header.php"); ?>
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        <div>
            <h4 class="mb-3 mb-md-0">ڈاؤن ٹرانزٹ ٹرک لوڈنگ اندراج </h4>
        </div>
        <div class="d-flex align-items-center flex-wrap text-nowrap">
            <?php echo backUrl('dt-truck-loading'); ?>
        </div>
    </div>
    <div class="row ">
                    ...
                    <div class="card-body">
                        ...
                        <form method="post">
                            ...
                            <?php if ($record['is_transfered'] == 0) { ?>
                                <a onclick="deleteRecord(this)" data-url="dt-truck-loading-add"
                                   data-tbl="dt_truck_loadings" id="<?php echo $record['id']; ?>"
                            ...
                            <div class="row gx-0 gy-5">
                                ...
                                <div class="col-lg-4">
                                    <div class="input-group">
                                        ...
                                        <label for="driver_mobile" class="input-group-text urdu">موبائل</label>
                                        <input type="text" id="driver_mobile" name="driver_mobile" tabindex="-1"
                                               class="form-control ltr bold" required readonly>
                                    </div>
                                </div>
                                <div class="col-lg-4 position-relative">
                                    <div class="input-group">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="input-group bg-info bg-opacity-10">
                                    <label for="ser" class="input-group-text urdu">سیریل نمبر</label>
                                    <input type="text" id="ser" class="form-control" disabled
                                           value="<?php echo getAutoIncrement('dt_truck_loadings'); ?>">
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="input-group bg-info bg-opacity-10">
...
<?php if (isset($_POST['recordSubmit'])) {
    /*$serial = fetch('dt_truck_loadings', array('branch_id' => $branchId));
    $branch_serial = mysqli_num_rows($serial);
    $branch_serial = $branch_serial + 1;*/
    $url = "dt-truck-loading-add";
    $data = array(
        'user_id' => $userId,
        'username' => $userName,
        'branch_id' => $branchId,
    ...
    $done = insert('dt_truck_loadings', $data);
    if ($done) {
        $insertId = $connect->insert_id;
        $url .= '?id=' . $insertId;
        message('success', $url, 'ڈاؤن ٹرانزٹ ٹرک لوڈنگ محفوظ ہوگیا');
    } else {
...
if (isset($_POST['recordUpdate'])) {
    ...
    $url = "dt-truck-loading-add?id=" . $hidden_id;
    $data = array(
        'owner_name' => mysqli_real_escape_string($connect, $_POST['owner_name']),
        'jins' => mysqli_real_escape_string($connect, $_POST['jins']),
        'tl_id' => mysqli_real_escape_string($connect, $_POST['tl_id']),
    ...
    if ($done) {
        message('success', $url, 'ڈاؤن ٹرانزٹ ٹرک لوڈنگ تبدیل ہوگیا');
    } else {
        message('danger', $url, 'ڈیٹابیس پرابلم');
    }
} ?>