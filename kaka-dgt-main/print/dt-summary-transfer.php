...
if (isset($_GET['type'])) {
    ...
    $typesAllowed = array('beopari_summary_dt', 'kiraya_summary_dt', 'godam_mazdoori_dt');
    ...
    if (in_array($getType, $typesAllowed)) {
        if (isset($_GET['id']) && ($_GET['id'] > 0) && isset($_GET['secret'])) {
            if (base64_decode($_GET['secret']) == "powered-by-upsol") {
                ...
                $urlArray = array(
                    'kiraya_summary_dt' => array('path' => 'dt-kiraya-summary', 'title' => 'ڈاؤن ٹرانزٹ کرایہ سمری', 'type' => 'ڈاؤن ٹرانزٹ کرایہ سمری', 'transfered_from' => 'kiraya_summary_dt', 'khaata_' => 'khaata_ks', 'back_url_type' => 'kiraya-summary'),
                    'beopari_summary_dt' => array('path' => 'dt-beopari-summary', 'title' => 'ڈاؤن ٹرانزٹ بیوپاری سمری', 'type' => 'ڈاؤن ٹرانزٹ بیوپاری سمری', 'transfered_from' => 'beopari_summary_dt', 'khaata_' => 'khaata_bs', 'back_url_type' => 'beopari-summary'),
                    'godam_mazdoori_dt' => array('path' => 'dt-godam-mazdoori-bill', 'title' => 'ڈاؤن ٹرانزٹ گودام مزدوری بل', 'type' => 'ڈاؤن ٹرانزٹ گودام مزدوری بل', 'transfered_from' => 'godam_mazdoori_dt', 'khaata_' => 'khaata_gm', 'back_url_type' => 'godam-mazdoori')
                );
                ...
                $infoArray = array('title' => $page['title'], 'row_cols' => 5,
                    'backUrl' => 'dt-summary-transfer?id=' . $id . '&type=' . $page['back_url_type'],
                                        ...
                                        <div class="card rounded-0 shadow-none border-0">
                                            ...
                                            <div class="card-body px-0 pt-0">
                                                <table class="table table-bordered">
                                                    ...
                                                    <?php $x = 1;
                                                    $total_bill = $bardana_qty = $per_wt = $total_wt = $empty_wt = $total_empty_wt = $saaf_wt = $total_expFinal = $taqseem_name = 0;
                                                    $maals = fetch('dt_truck_maals', array('dt_tl_id' => $id));
                                                    while ($maal = mysqli_fetch_assoc($maals)) {
                                                        ...
                                                        $total_bill += $total_exp;
                                                        echo '<tr style="font-weight: bold"><td colspan="4" align="left">ٹوٹل</td><td>' . $total_bill . '</td></tr>';
                                                    } ?>
                                                    ...
                                                    <div class="row pb-2 mt-3 border-top border-bottom border-dark border-2">
                                                        <div class="col">
                                                            <p class="urdu">
                                                                ...
                                                                <span class="ms-2"><?php echo $record['report']; ?></span>
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                ...
                <div class="sticky-social d-print-none">
                    <ul class="social">
                        ...
                        <li class="facebook" title="PDF پرنٹ کریں">
                    ...
                    <select class="virtual-select border" style="z-index: 9" id="summary-select">
                        ...
                        FROM dt_truck_loadings i
                        JOIN dt_truck_maals2 m ON i.id = m.dt_tl_id
...
<script>
    document.querySelector('#summary-select').addEventListener('change', function () {
        ...
        let urlString = "dt-summary-transfer?id=";
        let id = this.value;
...
<!--<script>
    function changeURL(obj) {
        window.$_GET = location.search.substr(1).split("&").reduce((o, i) => (u = decodeURIComponent, [k, v] = i.split("="), o[u(k)] = v && u(v), o), {});
        var urlString = "dt-summary-transfer?id=";
        var id = obj.options[obj.selectedIndex];
        urlString += id.value + '&secret=' + btoa('powered-by-upsol') + '&type=' + $_GET.type;
        window.location.href = urlString;
    }
</script>-->
