...
<?php
if (is_clearance_roznamcha_allowed('karachi', $userId)) {
...
/*if (Administrator()) {
    ...
    $jsonPerms = array();
    $perms = UTRoznamcha($userId);
    $url='./';
    if (empty($perms)) {
        message('danger', $url, 'ایڈمن کی طرف سے آپ کو یہ فارم اوپن کرنے کی اجازت نہیں ہے۔');
    } else {
        $perms = json_decode($perms);
        $jsonPerms = implode(',', $perms);
        $jsonPerms = explode(',', $jsonPerms);
        if ((in_array("karachi", $jsonPerms))) {
        } else {
            message("danger", $url, "ایڈمن کی طرف سے آپ کو یہ فارم اوپن کرنے کی اجازت نہیں ہے۔");
        }
    }
...
$sql = "SELECT * FROM roznamchaas WHERE r_type= 'ut_karachi' AND r_date = '$searchDate'";
//echo $sql;
...
<div class="heading-div d-flex justify-content-between align-items-center flex-wrap grid-margin px-4 py-1 border-bottom">
    <div>
        <h4 class="mt-n2 mb-3 mb-md-0">روزنامچہ کراچی</h4>
    </div>
