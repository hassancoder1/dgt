...
if ($_GET) {
    ...
    if ($id) {
        $r_custom_exp = fetch('r_custom_exp', array('id' => $id));
        $data = mysqli_fetch_assoc($r_custom_exp);
        ...
        if ($done) {
            ...
            update('r_custom_exp', $updateArray, array('id' => $id));
            message('success', $url, 'امپورٹ کسٹم خرچہ کاروبار روزنامچہ میں ٹرانسفر ہوگیا ہے۔' . $str);
        } else {
