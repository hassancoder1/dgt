...
if ($_POST) {
    ...
    $imp_truck_maals_id = mysqli_real_escape_string($connect, $_POST['imp_truck_maals_id']);
    $form_name = mysqli_real_escape_string($connect, $_POST['form_name']);

    $maal2 = isKirayaAdded($imp_truck_maals_id, $form_name);
    if ($maal2['success']) {//data already exists in maal2 table
        ...
        $done = update('imp_truck_maals2', $data, array('id' => $maal2Id));
        //$json2 = json_decode($maal2['output']['json_data']);
    } else {
        $data = array(
            'imp_tl_id' => $imp_tl_id,
            'maal_id' => $imp_truck_maals_id,
            'form_name' => $form_name,
            'json_data' => json_encode($_POST, JSON_UNESCAPED_UNICODE),
            'created_at' => date('Y-m-d H:i:s'),
            'created_by' => $userId
        );
        $done = insert('imp_truck_maals2', $data);
    }
    $message = messageAjax('info', ' تبدیلی ہو گئی ہے۔', 'mb-0');
    if ($done) {
        $maals = fetch('imp_truck_maals', array('imp_tl_id' => $_POST['imp_tl_id']));
        $x = 1;
        ...
        while ($maal = mysqli_fetch_assoc($maals)) {
            $maal2 = isKirayaAdded($maal['id'], $form_name);
            if ($maal2['success']) {
                $maal2Id = $maal2['output']['id'];
                $json2 = json_decode($maal2['output']['json_data']);
        ...
        $output .= '</tr>';
        $output .= '<tr class="row-py-0"><td class="p-0" colspan="12">' . $message . '</td></tr>';
    } else {
        $message = messageAjax('danger', 'ڈیٹا بیس پرابلم');
    }
    echo $output;
    /*echo $message;    echo '<pre>';var_dump($data['json_data']);*/
} ?>
