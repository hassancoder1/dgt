...
if ($_POST['imp_truck_maals_action']) {
    $output = '';
    $message = '';
    $done = false;
    $action = $_POST['imp_truck_maals_action'];
    if ($action == "insert") {
        $data = array(
            'imp_tl_id' => $_POST['imp_tl_id'],
            'json_data' => json_encode($_POST, JSON_UNESCAPED_UNICODE),
            'created_by' => $userId,
            'created_at' => date('Y-m-d H:i:s')
        );
        $done = insert('imp_truck_maals', $data);
    ...
    if ($action == "update") {
        ...
        $done = update('imp_truck_maals', $data, array('id' => $_POST['imp_truck_maals_id']));
    ...
    if ($done) {
        ...
        $output .= '</tr>';
        $output .= '<tr class="row-py-0 p-0"><td class="p-0" colspan="9">' . $message . '</td></tr>';
    } else {
