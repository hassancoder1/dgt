...
if ($_GET) {
    ...
    if ($id) {
        $r_office_exp = fetch('r_office_exp', array('id' => $id));
        $data = mysqli_fetch_assoc($r_office_exp);
        ...
        $r_type = "karobar";
        $transfered_from = "r_office_exp";

        $bill_giver = ' بل دینے والا نام ' . $data['bill_giver'] . ' ';
        $details = ' آفس خرچہ ٹرانسفر ';
        $dataArray = array(
            'r_type' => $r_type,
        ...
        for ($i = 1; $i <= 2; $i++) {
            if ($i == 1) {
                ...
                $dataArray['details'] = '  بنام: ' . $data['bnaam_khaata_no'] . $bill_giver . $details;
            ...
            if ($i == 2) {
                ...
                $dataArray['details'] = '  جمع: ' . $data['jmaa_khaata_no'] . $bill_giver . $details;
        ...
        if ($done) {
            ...
            update('r_office_exp', $updateArray, array('id' => $id));
            message('success', $url, 'آفس خرچہ کاروبار روزنامچہ میں ٹرانسفر ہوگیا ہے۔' . $str);
        } else {
