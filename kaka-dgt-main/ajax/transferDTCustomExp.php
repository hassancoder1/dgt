...
if ($_GET) {
    ...
    if ($id) {
        $r_dt_custom_exp = fetch('r_dt_custom_exp', array('id' => $id));
        $data = mysqli_fetch_assoc($r_dt_custom_exp);
        ...
        $r_type = "karobar";
        $transfered_from = "dt_custom_exp";
        $details = ' کنسائینی شہر: ' . $data['sender_city'];
        $details .= ' کنسائینی نام: ' . $data['sender_name'];
        $details .= ' ڈاون ٹرانزٹ کسٹم خرچہ ٹرانسفر ';
        $dataArray = array(
            'r_type' => $r_type,
        ...
        if ($done) {
            ...
            update('r_dt_custom_exp', $updateArray, array('id' => $id));
            message('success', $url, 'ڈاون ٹرانزٹ کسٹم خرچہ کاروبار روزنامچہ میں ٹرانسفر ہوگیا ہے۔' . $str);
        } else {
