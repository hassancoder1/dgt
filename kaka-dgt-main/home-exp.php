...
$sql = "SELECT * FROM r_home_exp WHERE id > 0 ";
if (!Administrator()) {
    $sql .= "AND branch_id = " . "'$branchId'" . " ";
}
if ($_POST) {
    $removeFilter = removeFilter('home-exp');
...
<div class="heading-div d-flex justify-content-between align-items-center flex-wrap grid-margin px-4 py-1 border-bottom">
    <div>
        <h4 class="mb-3 mb-md-0">گھر کا خرچہ</h4>
    </div>
                    ...
                    <table class="table table-bordered " id="fix-head-table">
                        ...
                        <?php if (mysqli_num_rows($records) > 0) {
                            while ($roz = mysqli_fetch_assoc($records)) { ?>
                                <tr>
                                    <td>
                                        <a href="home-exp-add?id=<?php echo $roz["id"]; ?>"><?php echo $roz["id"]; ?></a>
                                        <?php if ($roz['is_transfered'] == 0) { ?>
                                            <a class="btn btn-primary py-0 px-1 small float-end" onclick="transferHomeExp(this)"
                                               id="<?php echo $roz['id']; ?>" data-url="home-exp"
                                               data-amount="<?php echo $roz['total_bill']; ?>">ٹرانسفر </a>
                                        <?php }else {
                                            echo '<a class="btn btn-primary pt-0 p-1 mt-n1 btn-sm float-end"
                                           data-tooltip="گھر کا خرچہ پرنٹ" data-tooltip-position="left"
                                           href="print/bill_roznamcha?id=' . $roz["id"] . '&secret=' . base64_encode("powered-by-upsol") . '&url=' . base64_encode("ut-commission-border-bill") . '&type=' . BR_HOME_EXP. '"><i class="fa fa-print"></i>
                                        </a>';
                                        } ?>
                                    </td>
                                    <?php if (Administrator()) { ?>
...
<script>
    function transferHomeExp(e) {
        var id = $(e).attr('id');
        var amount = $(e).attr('data-amount');
        var url = $(e).attr('data-url');
        if (id) {
            if (confirm('کیا آپ ٹرانسفرکرنا چاہتے ہیں؟\n سیریل #: ' + id + '\n رقم: ' + amount)) {
                window.location.href = 'ajax/transferHomeExp.php?id=' + id + '&amount=' + amount + '&url=' + url;
            } else {
