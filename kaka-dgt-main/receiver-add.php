...
<div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
    <div>
        <h4 class="mb-3 mb-md-0">مال وصول کرنے والا اندراج</h4>
    </div>
    <div class="d-flex align-items-center flex-wrap text-nowrap">
        <a href="receivers.php"
           class="btn btn-dark btn-icon-text mb-2 mb-md-0">
            <i class="btn-icon-prepend" data-feather="arrow-left-circle"></i>مال وصول کرنے والا
            تفصیل</a>
    </div>
</div>
<div class="row">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <?php if (isset($_GET['id'])) {
                    ...
                    $records = fetch('receivers', array('id' => $id));
                    ...
                    <form action="" method="post">
                        ...
                        <div class="d-flex mt-4 align-items-center justify-content-between">
                            <button type="submit" name="recordUpdate" class="btn btn-dark btn-icon-text">
                                <i class="btn-icon-prepend" data-feather="edit-3"></i>
                                درستگی
                            </button>
                            <div><?php echo addNew('receiver-add'); ?></div>
                        </div>
                    </form>
                <?php } else { ?>
                    <form action="" method="post">
...
if (isset($_POST['recordSubmit'])) {
    ...
    $done = insert('receivers', $data);
    if ($done) {
        $url .= '?id=' . $connect->insert_id;
        message('success', $url, 'مال وصول کرنے والا اندراج ہوگیا');
    } else {
...
if (isset($_POST['recordUpdate'])) {
    ...
    $done = update('receivers', $data, array('id' => $hidden_id));
    if ($done) {
        message('success', $url, 'مال وصول کرنے والا تبدیل ہوگیا');
    } else {
        message('danger', $url, 'ڈیٹابیس پرابلم');
    }
