...
<?php $searchUserName = $date_msg = $username_msg = $removeFilter = "";
$start_date = $end_date = date('Y-m-d');
$sql = "SELECT * FROM ut_bail_entries WHERE is_surrender = 1 AND qandhar_user_ids IS NOT NULL ";
if ($_POST) {
...
<div class="heading-div d-flex justify-content-between align-items-center flex-wrap grid-margin px-4 py-1 border-bottom">
    <div><h3 class="mb-3 mb-md-0 urdu-2"> خالی کرنے گودام </h3></div>
                ...
                <div class="table-responsive scroll screen-ht">
                    <table class="table table-bordered " id="fix-head-table">
                        ...
                        <?php while ($loading = mysqli_fetch_assoc($records)) {
                            /*echo '<pre>';
                            var_dump($loading);*/
                            $rowClass = empty($loading['godam_json']) ? 'bg-danger bg-opacity-25 border border-white' : '';
                            //if (!empty($loading['khaata_qandhar'])) { ?>
                                <tr class="<?php echo $rowClass; ?>">
                                    <td class="text-nowrap">
                                        <a class="h5" data-tooltip="بیل کی تفصیل"
                                           data-tooltip-position="bottom left"
                                           href="ut-godam-empty-add?id=<?php echo $loading["id"]; ?>"><?php echo $loading["id"]; ?></a>
                                        <?php if (empty($loading['godam_json'])) {
                                            echo '<i class="fa fa-warning text-danger float-end me-2 mt-1" data-tooltip="بیل نمبر ' . $loading["id"] . ' میں ابھی خالی کرنے گودام کی انٹری نہیں ہوئی۔ " data-tooltip-position="right"></i>'; ?>
                                        <?php }
                                        $t_f_g = $loading['transfer_from_godam'];
                                        switch ($t_f_g) {
                                            case 1:
                                                echo '<a href="ut-expense-transfer?id=' . $loading["id"] . '&type=' . BORDER_BILL . '" class="small-2 btn btn-primary py-0 px-1 small urdu-2 float--end">بارڈر بل</a>';
                                                break;
                                            case 2:
