...
$selectedBranchId = 0;
$sql = "SELECT * FROM r_custom_exp WHERE id > 0 ";
if (!Administrator()) {
    $sql .= "AND branch_id = " . "'$branchId'" . " ";
}
if ($_POST) {
    $removeFilter = removeFilter('import-kharcha');
...
<div class="heading-div d-flex justify-content-between align-items-center flex-wrap grid-margin px-4 py-1 border-bottom">
    <div>
        <h4 class="mb-3 mb-md-0">امپورٹ کسٹم خرچہ</h4>
    </div>
                    ...
                    <table class="table table-bordered " id="fix-head-table">
                        ...
                        if (mysqli_num_rows($records) > 0) {
                            while ($roz = mysqli_fetch_assoc($records)) { ?>
                                <tr>
                                    <td>
                                        <a href="import-kharcha-add?id=<?php echo $roz["id"]; ?>"><?php echo $roz["id"]; ?></a>
                                        <?php if ($roz['is_transfered'] == 0) { ?>
                                            <a class="btn float-end btn-primary py-0 px-1 small"
                                               onclick="transferCustomExp(this)"
                                               id="<?php echo $roz['id']; ?>" data-url="import-kharcha"
                                               data-amount="<?php echo $roz['total_bill']; ?>">ٹرانسفر </a>
                                        <?php } else {
                                            echo '<a class="btn btn-primary pt-0 p-1 mt-n1 btn-sm float-end"
                                           data-tooltip="امپورٹ کسٹم خرچہ کا پرنٹ" data-tooltip-position="left"
                                           href="print/bill_roznamcha?id=' . $roz["id"] . '&secret=' . base64_encode("powered-by-upsol") . '&url=' . base64_encode("ut-commission-border-bill") . '&type=' . BR_IMPORT_CUSTOM_EXP . '"><i class="fa fa-print"></i>
                                        </a>';
                                        } ?>
                                    </td>
                                    <?php if (Administrator()) { ?>
...
<script>
    function transferCustomExp(e) {
        var id = $(e).attr('id');
        var amount = $(e).attr('data-amount');
        var url = $(e).attr('data-url');
        if (id) {
            if (confirm('کیا آپ ٹرانسفرکرنا چاہتے ہیں؟\n سیریل #: ' + id + '\n رقم: ' + amount)) {
                window.location.href = 'ajax/transferCustomExp.php?id=' + id + '&amount=' + amount + '&url=' + url;
            } else {
