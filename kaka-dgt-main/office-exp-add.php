<?php include("header.php"); ?>
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        <div>
            <h4 class="mb-3 mb-md-0 mt-n2">آفس خرچہ اندراج </h4>
        </div>
        <div class="d-flex align-items-center flex-wrap text-nowrap">
            <?php echo backUrl('office-exp'); ?>
        </div>
    </div>
    <div class="row">
        <div class="col-md-10">
            <div class="card">
                <div class="card-body">
                    ...
                    <?php if (isset($_GET['id'])) {
                        ...
                        $records = fetch('r_office_exp', array('id' => $id));
                        ...
                        <form action="" method="post">
                                        ...
                                        <div class="col-md-12">
                                            <div class="input-group bg-info bg-opacity-10">
                                                ...
                                                <input type="text" id="ser" class="form-control" disabled
                                                       value="<?php echo getAutoIncrement('r_office_exp'); ?>">
                                            </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div class="input-group bg-info bg-opacity-10">
                            ...
                            <?php if ($record['is_transfered'] == 0) { ?>
                                <!--<a onclick="deleteRecord(this)" data-url="office-exp-add"
                                   data-tbl="r_office_exp" id="<?php /*echo $record['id']; */?>"
...
<?php if (isset($_POST['recordSubmit'])) {
    $serial = fetch('r_office_exp', array('branch_id' => $branchId));
    ...
    $done = insert('r_office_exp', $data);
    if ($done) {
        $inserId = $connect->insert_id;
        $url .= '?id=' . $inserId;
        message('success', $url, 'آفس خرچہ محفوظ ہوگیا');
    } else {
...
if (isset($_POST['recordUpdate'])) {
    ...
    $url = "office-exp-add?id=" . $hidden_id;
    $exp_names = json_encode($_POST['exp_names'], JSON_UNESCAPED_UNICODE);
    $exp_details = json_encode($_POST['exp_details'], JSON_UNESCAPED_UNICODE);
    $exp_values = json_encode($_POST['exp_values']);
    $data = array(
    ...
    $done = update('r_office_exp', $data, array('id' => $hidden_id));
    if ($done) {
        message('success', $url, 'آفس خرچہ تبدیل ہوگیا');
    } else {
        message('danger', $url, 'ڈیٹابیس پرابلم');
    }
} ?>