<?php include("header.php"); ?>
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        <div>
            <h4 class="mb-3 mb-md-0">ڈاون ٹرانزٹ کسٹم خرچہ اندراج </h4>
        </div>
        <div class="d-flex align-items-center flex-wrap text-nowrap">
            <?php echo backUrl('dt-custom-kharcha'); ?>
        </div>
    </div>
    <div class="row">
        <div class="col-md-10">
            <div class="card">
                <div class="card-body">
                    ...
                    <?php if (isset($_GET['id'])) {
                        ...
                        $records = fetch('r_dt_custom_exp', array('id' => $id));
                        ...
                        <form action="" method="post">
                                        ...
                                        <div class="col-md-12">
                                            <div class="input-group bg-info bg-opacity-10">
                                                ...
                                                <input type="text" id="ser" class="form-control" disabled
                                                       value="<?php echo getAutoIncrement('r_dt_custom_exp'); ?>">
                                            </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div class="input-group bg-info bg-opacity-10">
                            ...
                            <?php if ($record['is_transfered'] == 0) { ?>
                                <!--<a onclick="deleteRecord(this)" data-url="dt-custom-kharcha-add"
                                   data-tbl="r_dt_custom_exp" id="<?php /*echo $record['id']; */?>"
...
<?php if (isset($_POST['recordSubmit'])) {
    $serial = fetch('r_dt_custom_exp', array('branch_id' => $branchId));
    $branch_serial = mysqli_num_rows($serial);
    $branch_serial = $branch_serial + 1;
    $url = "dt-custom-kharcha-add";
    ...
    $done = insert('r_dt_custom_exp', $data);
    if ($done) {
        message('success', $url, 'ڈاون ٹرانزٹ کسٹم خرچہ محفوظ ہوگیا');
    } else {
...
if (isset($_POST['recordUpdate'])) {
    ...
    $url = "dt-custom-kharcha-add?id=" . $hidden_id;
    $exp_names = json_encode($_POST['exp_names'], JSON_UNESCAPED_UNICODE);
    $exp_details = json_encode($_POST['exp_details'], JSON_UNESCAPED_UNICODE);
    $exp_values = json_encode($_POST['exp_values']);
    $data = array(
    ...
    $done = update('r_dt_custom_exp', $data, array('id' => $hidden_id));
    if ($done) {
        message('success', $url, 'ڈاون ٹرانزٹ کسٹم خرچہ تبدیل ہوگیا');
    } else {
        message('danger', $url, 'ڈیٹابیس پرابلم');
    }
} ?>